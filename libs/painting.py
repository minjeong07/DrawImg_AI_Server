...
class Painting:
    def __init__(self, url):
        ...
        self.url = np.asarray(bytearray(requests.get(url).content), dtype=np.uint8)
        self.image = cv2.imdecode(self.url, cv2.IMREAD_COLOR)
        self.image = cv2.cvtColor(self.image, cv2.COLOR_BGR2RGB)
        self.fileBasename = os.path.basename(url) # file base name
...
def imageExpand(image, guessSize=False, size = 3):
    ...
    image = cv2.resize(image, dsize=(2480,3508), interpolation=cv2.INTER_LINEAR)
